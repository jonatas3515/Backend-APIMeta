...
O **Neves & Costa Chat System** é uma plataforma Next.js + Supabase + Vercel já em produção, com chat WhatsApp, gestão de casos, agenda, templates, insights, métricas, base de conhecimento RAG e módulo de acompanhamento processual DataJud. O sistema possui **53 migrations** (incluindo Central de Triagem Processual), autenticação baseada em JWT/Supabase, RLS e fluxo de consentimento LGPD. Atualmente há **4 documentos RAG aprovados** e ativos. **Testes automatizados implementados:** 26 testes de fumaça cobrindo fluxos críticos (autenticação, webhook LGPD, RAG, DataJud, triagem) com Jest + Testing Library, todos os testes passando, sem chamadas externas reais e com dados sintéticos. Cobertura atual: lib 0.22%, pages/api 0% (critério de aceite é proteger fluxos críticos, não cobertura global). A principal ressalva técnica é o **DataJud com autenticação rejeitada (HTTP 401)**: a variável `DATAJUD_API_KEY` existe no Vercel, mas a API pública não aceitou a chave em nenhum dos esquemas testados (`APIKey` e `Basic`). Também confirmou-se **exposição de PII nos logs do webhook** (`pages/api/webhook.js` imprime body, headers, telefone, nome e texto). Outros pontos críticos: revisão manual de anonimização de nomes, rotação de chaves temporárias e acompanhamento das vulnerabilidades de dependências documentadas. O build e o deploy estão funcionando.
