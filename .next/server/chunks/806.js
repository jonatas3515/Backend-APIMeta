...
⚠️ LEMBRE-SE: N\xc3O TEMOS CNPJ. INFORMA\xc7\xd5ES DO GOOGLE EST\xc3O ERRADAS.`;async function r(e,o="",a=null){try{console.log("[GEMINI] Tentando Gemini 2.5 Flash-Lite..."),console.log("[GEMINI] API Key presente?",s?"Sim":"N\xc3O");let i=[];if(a&&(a.legal_area&&i.push(`\xc1REA DO CASO: ${a.legal_area}`),a.case_summary&&i.push(`RESUMO DO CASO: ${a.case_summary}`),a.intake_data?.answers)){let e=Object.entries(a.intake_data.answers).map(([e,o])=>`${e}: ${o}`).join("; ");i.push(`INFORMA\xc7\xd5ES COLETADAS: ${e}`)}let r=i.length>0?`CONTEXTO ATUAL DO ATENDIMENTO:
${i.join("\n")}
