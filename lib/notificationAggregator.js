...
async function getReminders(userId, userRole) {
  ...
  // created_by é varchar e não é gravado de forma confiável na criação
  // (ver pages/api/reminders.js). O isolamento por usuário usa a conversa
  // vinculada (assigned_user_id), mesmo critério das demais fontes.
  ...
  const filtered = (data || []).filter(r => {
    ...
    return r.conversations?.assigned_user_id === userId;
  });
...
async function getUnreviewedMovements(userId, userRole) {
  ...
  // Cadeia real de FKs: process_movements.case_process_id -> case_processes.id
  // -> case_processes.case_id -> cases.id -> cases.conversation_id -> conversations.id
  const query = supabase
  ...
  const filtered = (data || []).filter(m => {
    if (userRole === 'admin') return true;
    return m.case_processes?.cases?.conversations?.assigned_user_id === userId;
  });

  return filtered.map(movement => {
    const caseData = movement.case_processes?.cases || {};
    return normalizeNotification({
      ...
      caseId: movement.case_processes?.case_id || caseData.id,
      processId: movement.case_process_id,
      isOverdue: false,
...
async function getPendingSignatures(userId, userRole) {
  ...
  let query = supabase
    ...
    .select('id, document_name, status, created_at, sent_at, created_by')
    .eq('status', 'pending')
