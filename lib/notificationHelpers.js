...
export function getNotificationRoute(notification) {
  ...
  switch (type) {
    ...
    case 'reminder_overdue':
      if (conversationId || reference_type === 'conversation') {
        const convId = conversationId || reference_id;
        return buildInternalUrl({ tab: 'chat', conversationId: convId, reminderId: reference_id });
      }
      if (caseId || reference_type === 'case') {
        const cid = caseId || reference_id;
        return buildInternalUrl({ tab: 'cases', caseId: cid, reminderId: reference_id });
      }
