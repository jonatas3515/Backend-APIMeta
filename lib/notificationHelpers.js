...
/**
 * Allowlist de rótulos seguros para fontes de notificações indisponíveis.
 * Nunca expõe nome técnico, SQL, endpoint, code, stack trace ou PII.
 */
const SAFE_SOURCE_LABELS = {
  messages: 'Mensagens',
  reminders: 'Lembretes',
  deadlines: 'Prazos e eventos',
  events: 'Prazos e eventos',
  cases: 'Casos',
  process_movements: 'Atualizações processuais',
  signatures: 'Assinaturas'
};

export function getSafeSourceLabel(source) {
  if (!source || typeof source !== 'string') return null;
  return SAFE_SOURCE_LABELS[source] || null;
}

export function getSafeSourceLabels(errors = []) {
  const labels = new Set();
  const safeErrors = Array.isArray(errors) ? errors : [];

  for (const err of safeErrors) {
    if (!err || !err.source) continue;
    const label = getSafeSourceLabel(err.source);
  ...
  if (list.length > 3 || safeErrors.length > 3) {
    return 'Algumas fontes de atualização estão indisponíveis.';
  }
