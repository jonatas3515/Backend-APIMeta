...
export async function parsePdfTable(buffer) {
  ...
  for (const page of pages) {
    const lines = cleanPageText(page);
  ...
  if (!headers) {
    // Se nao identificou cabecalho, cria colunas genericas com base na primeira linha
    const maxCols = allRows.reduce((max, row) => Math.max(max, row.length), 0);
    headers = Array.from({ length: maxCols }, (_, i) => `col${i}`);
  }
