...
async function transcribeWithWhisper(audioBuffer) {
  try {
    ...
    console.log('[MEDIA] Áudio transcrito via Whisper, comprimento:', text?.length || 0);
    return text || null;
  } catch (error) {
...
async function transcribeWithGemini(audioBuffer, mimeType = 'audio/ogg') {
  ...
  try {
    ...
    console.log('[MEDIA] Áudio transcrito via Gemini, comprimento:', text?.length || 0);
    return text || null;
  } catch (error) {
...
export async function summarizeMedia(mediaUrl, mimeType) {
  ...
  try {
    ...
    const data = await response.json();
    const text = data.candidates?.[0]?.content?.parts?.[0]?.text;
    console.log('[MEDIA] Mídia resumida, comprimento:', text?.length || 0);
    return text || null;
  } catch (error) {
