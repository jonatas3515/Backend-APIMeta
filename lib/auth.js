  ...
  return async (req, res) => {
    try {
      ...
      if (options.minRole && !hasMinimumRole(profile, options.minRole)) {
        console.warn('[AUTH] Acesso negado - papel insuficiente:', profile.role, 'requer:', options.minRole);
      ...
      if (options.allowedRoles && !options.allowedRoles.includes(profile.role)) {
        console.warn('[AUTH] Acesso negado - papel não permitido:', profile.role);
        return res.status(403).json({
          error: `Acesso negado. Requer um dos papéis: ${options.allowedRoles.join(', ')}`
        });
      }

      console.log('[AUTH] Usuário autenticado:', 'role:', profile.role);
      req.user = profile;
