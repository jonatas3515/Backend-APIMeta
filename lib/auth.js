...
export async function verifyAuthAndGetUser(req) {
  ...
  // console.log('[AUTH] Token extraído: [REDACTED]');
  ...
  return async (req, res) => {
    try {
      ...
      if (options.minRole && !hasMinimumRole(profile, options.minRole)) {
        console.warn('[AUTH] access_denied role=' + profile.role + ' required=' + options.minRole);
      ...
      if (options.allowedRoles && !options.allowedRoles.includes(profile.role)) {
        console.warn('[AUTH] access_denied role=' + profile.role + ' allowed=' + (options.allowedRoles ? options.allowedRoles.join(',') : ''));
        return res.status(403).json({
          error: `Acesso negado. Requer um dos papéis: ${options.allowedRoles.join(', ')}`
        });
      }

      console.log('[AUTH] authenticated role=' + profile.role);
      req.user = profile;
      return await handler(req, res);
    } catch (error) {
      console.error('[AUTH] authentication_failed:', error.message);
