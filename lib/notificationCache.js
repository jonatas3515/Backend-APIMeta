...
class NotificationCache {
   ...
   * @returns {String}
   */
  _getKey(userId, type = 'list', role = 'unknown') {
    return `notifications:${userId}:${role}:${type}:v1`;
  }
   ...
   * @returns {Any|null}
   */
  get(userId, type = 'list', role = 'unknown') {
    const key = this._getKey(userId, type, role);
   ...
   * @param {String} type - Tipo de cache
   */
  set(userId, data, type = 'list', role = 'unknown') {
    const key = this._getKey(userId, type, role);
   ...
   * @param {String} userId - ID do usuário
   */
  invalidate(userId, role = null) {
    if (role) {
      this.cache.delete(this._getKey(userId, 'list', role));
      this.cache.delete(this._getKey(userId, 'count', role));
      return;
    }
