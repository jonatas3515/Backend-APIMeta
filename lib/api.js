...
export async function getAuthHeaders() {
  try {
    ...
    if (session && isTokenAboutToExpire(session)) {
      console.log('[API] Token perto de expirar, tentando refresh...');
      ...
      if (refreshPromise) {
        ...
        try {
          ...
          session = refreshData?.session;
          console.log('[API] Token renovado:', session ? 'SIM' : 'NÃO');
        } finally {
          refreshPromise = null;
        }
      }
    ...
    if (session?.access_token) {
      console.log('[API] Token usado:', `${session.access_token.substring(0, 20)}...`);
