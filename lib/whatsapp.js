...
export async function uploadMediaToWhatsApp(fileBuffer, mimeType) {
  try {
    ...
    if (!response.ok) {
      ...
      console.error(`[WHATSAPP] Corpo: ${errorBody}`);
      throw new Error(`Erro ao fazer upload de mídia: ${response.status} ${response.statusText} - ${errorBody}`);
    }
...
export async function sendWhatsAppMediaMessage(to, mediaId, mediaType, caption = '') {
  try {
    console.log(`[WHATSAPP] Enviando mídia ${mediaType} para ${to}. Media ID: ${mediaId}`);
    ...
    if (!response.ok) {
      ...
      console.error(`[WHATSAPP] Corpo: ${errorBody}`);
      throw new Error(`Erro ao enviar mídia WhatsApp: ${response.status} ${response.statusText} - ${errorBody}`);
    }

    const data = await response.json();
    console.log(`[WHATSAPP] ✅ Mídia enviada com sucesso. ID: ${data.messages?.[0]?.id}`);
  } catch (error) {
