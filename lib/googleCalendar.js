...
export async function createGoogleEvent({ supabaseAdmin, userId, event }) {
  ...
  if (!response.ok) {
    if (response.status === 401 || response.status === 403 || data.error?.code === 401 || data.error?.code === 403) {
      ...
      const err = new Error('Sua conexão com o Google Calendar expirou. Conecte novamente.');
...
export async function updateGoogleEvent({ supabaseAdmin, userId, eventId, event }) {
  ...
  if (!response.ok) {
    ...
    if (response.status === 404 || data.error?.code === 404) {
      const err = new Error('Evento não encontrado no Google Calendar. Será recriado.');
      err.code = 'GOOGLE_EVENT_NOT_FOUND';
      throw err;
    }
